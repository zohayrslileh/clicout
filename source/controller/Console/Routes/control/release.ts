import application, { Package } from "@/Models/Config/package"
...
export default async function ({ flags }: Arguments) {
    ...
    // Release path
    const releasePath = join("release", application.version)
    ...
    // Copy dependencies
    if (application.dependencies && flags.withDeps) terminal.execute("npm install --prefer-offline --no-package-lock", releasePath)

    // Compress
    await new Navigator(releasePath).compress(join("release", `${application.name}@${application.version}.zip`))
